...
```url
[POST] https://relay.h.crynux.ai/v1/inference_tasks
```
...
A reference implementation of the signing method, in `Golang`, can be found in the [here](https://github.com/crynux-ai/ig-server/blob/main/relay/sign\_data.go). The code to upload the task arguments to the Relay can also be found [in the source code](https://github.com/crynux-ai/ig-server/blob/main/relay/inference\_task.go).
...
```url
[GET] https://relay.h.crynux.ai/v1/inference_tasks/<:task_id>/results/<:image_number>
```

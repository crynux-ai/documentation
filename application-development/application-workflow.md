...
A reference implementation of the signing method, in `Golang`, can be found in the [here](https://github.com/crynux-ai/ig-server/blob/main/relay/sign\_data.go). The complete code to upload the task arguments to the Relay can also be found [in the source code](https://github.com/crynux-ai/ig-server/blob/main/relay/inference\_task.go).
